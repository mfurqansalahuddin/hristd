...
class TooltipManager {
  ...
  createTooltipElement(content, variant) {
    ...
    const borderClass =
      variant === "plain" || variant === "no-arrow" ? "" : "border";

    tooltip.className = `hidden absolute z-99999 whitespace-nowrap rounded-lg ${borderClass} px-3.5 py-2 text-xs font-medium shadow-md ${variantClasses}`;

    const noArrow = variant === "plain" || variant === "no-arrow";

    tooltip.innerHTML = noArrow
      ? content
      : `${content}<div class="tooltip-arrow absolute h-3 w-3"></div>`;

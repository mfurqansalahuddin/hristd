...
<div class="relative" x-data="{
    ...
    <button class="flex items-center text-gray-700 dark:text-gray-400" @click.prevent="toggleDropdown()" type="button">
        <span class="mr-3 overflow-hidden rounded-full h-11 w-11">
            <img src="{{ $authUser->photoUrl() }}" alt="{{ $authUser->name }}" class="h-full w-full object-cover" />
        </span>

        <span class="block mr-1 font-medium text-theme-sm">{{ \Illuminate\Support\Str::before($authUser->name, ' ') }}</span>
    ...
    <div x-show="dropdownOpen" x-transition:enter="transition ease-out duration-100"
        ...
        <!-- User Info -->
        <div>
            <span class="block font-medium text-gray-700 text-theme-sm dark:text-gray-400">{{ $authUser->name }}</span>

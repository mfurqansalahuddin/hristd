...
<div
  x-data="{ 
    selectedItems: [],
    rows: @js(array_column($transactions, 'id')),
    toggleAll() {
...
>
  <div
    class="mb-4 flex flex-col gap-5 px-6 sm:flex-row sm:items-center sm:justify-between"
  >
    <div>
  ...
  <div class="custom-scrollbar max-w-full overflow-x-auto">
    <table class="min-w-full">
      <thead
        class="border-y border-gray-100 bg-gray-50 dark:border-gray-800 dark:bg-gray-900"
      >
        <tr>
            ...
            <div class="flex items-center">
              <div class="flex items-center gap-3">
                <div
                  ...
                  :class="selectedItems.length === rows.length ? 'border-brand-500 dark:border-brand-500 bg-brand-500' : 'bg-white dark:bg-white/0 border-gray-300 dark:border-gray-700' "
                >
                  <svg
                    :class="selectedItems.length === rows.length ? 'block' : 'hidden'"
                    width="14"
                    height="14"
                    viewBox="0 0 14 14"
                    fill="none"
                ...
                <div>
                  <span
                    class="text-theme-xs block font-medium text-gray-500 dark:text-gray-400"
                  >
                    Order ID
                  </span>
                </div>
        ...
        <tr>
          <td class="px-6 py-4 whitespace-nowrap">
            <div class="flex items-center">
              <div class="flex items-center gap-3">
                <div
                  @click="toggleItem('{{ $transaction['id'] }}')"
                  class="flex h-5 w-5 cursor-pointer items-center justify-center rounded-md border-[1.25px]"
                  :class="selectedItems.includes('{{ $transaction['id'] }}') ? 'border-brand-500 dark:border-brand-500 bg-brand-500' : 'bg-white dark:bg-white/0 border-gray-300 dark:border-gray-700' "
                >
                  <svg
                    :class="selectedItems.includes('{{ $transaction['id'] }}') ? 'block' : 'hidden'"
                    width="14"
                    height="14"
                    viewBox="0 0 14 14"
                    fill="none"
          ...
          <td class="px-6 py-4 whitespace-nowrap">
            <p
              class="text-theme-xs inline-flex rounded-full px-2 py-0.5 font-medium {{ $transaction['status_type'] === 'success' ? 'bg-success-50 text-success-600 dark:bg-success-500/15 dark:text-success-500' : 'bg-warning-50 text-warning-600 dark:bg-warning-500/15 dark:text-warning-400' }}"
            >
              {{ $transaction['status'] }}
            </p>
          </td>

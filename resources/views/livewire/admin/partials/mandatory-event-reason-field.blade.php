...
@if ($editing)
    <div class="mb-1 flex items-center justify-between gap-2">
        ...
        @if (filled($this->edits[$participant->id]['reason'] ?? null) && ! $this->isLocked())
    ...
    <textarea wire:model="edits.{{ $participant->id }}.reason" rows="2" @disabled($this->isLocked())
        placeholder="{{ $status === 'TIDAK_HADIR' ? 'Wajib diisi, mis. Sakit / Dinas Luar / Piket Malam' : 'Opsional, catatan keterlambatan' }}"
        class="dark:bg-dark-900 shadow-theme-xs w-full min-w-[200px] rounded-lg border border-gray-300 bg-transparent px-3 py-2 text-sm text-gray-800 disabled:opacity-60 dark:border-gray-700 dark:text-white/90"></textarea>
